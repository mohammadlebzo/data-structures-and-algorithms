...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let tempTest = false;
  Object.entries(arr).forEach(element => element[1].children && element[1].name.includes(character) ? tempTest = true : tempTest );
  return tempTest;
};
...
const houseSize = (arr) => {
  ...
  Object.values(arr).forEach(element => {
    if (element.children){
      sizes.push({'house': element.house, 'members': 2 + element.children.length}) 
    } else if(element.spouse) {
      sizes.push({'house': element.house, 'members': 2})
    }
    else{
...
const houseSurvivors = (arr) => {
  ...
  Object.values(arr).forEach(element => {
    if (element.children){
      ...
      deceasedSpouses.forEach(elementD => elementD === element.spouse ? isDeceased = true : isDeceased);
      if(isDeceased){
        survivors.push({'house': element.house, 'members': 1 + element.children.length});
      }
      else{
        survivors.push({'house': element.house, 'members': 2 + element.children.length});
      }
    } else if(element.spouse) {
      let isDeceased = false;
      deceasedSpouses.forEach(elementD => elementD === element.spouse ? isDeceased = true : isDeceased);
      if(isDeceased){
        ...
        survivors.push({'house': element.house, 'members': 2})
      }
    }
    else{
      survivors.push({'house': element.house, 'members': 1})
